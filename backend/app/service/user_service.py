...
class UserService:
    ...
    def get_user_insights(self, db: Database, user_id: int) -> InsightDTO:
        ...
        for review in user.reviews:
            print(f"Rating ID: {review.id}, Score: {review.rating}, Movie ID: {review.movie.id}, Movie Title: {review.movie.title}")
            ...
            if review.rating is not None:
                total_sum_ratings += review.rating
                total_rated_movies += 1
                
            total_movies_watched += 1
            total_time_watched += review.movie.duration
